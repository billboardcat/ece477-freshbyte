...
def update_sensors_IFTTT(_):
    ...
    sensor_readings = {'value1': session_str + '|||' + str(temp) + 'F',
                       'value2': str(humid) + '%',
                       'value3': str(methane)
                       }
    ...
    try:
        ...
        request = urequests.post(
            'http://maker.ifttt.com/trigger/ece477/with/key/' + api_key,
...
def init_IFTTT_timer():
    ...
    # callback every 6 seconds
    # TODO - every min.
    sensor_update_timer.init(period=6000, mode=Timer.PERIODIC, callback=update_sensors_IFTTT)
